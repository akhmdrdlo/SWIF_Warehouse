    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder mb-0 text-white">Tambah Pengiriman</h6>
        </nav>
        <nav>
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header pb-0">
                  <div class="d-flex">
                    <h5 class="mb-0">Kirim Barang</h5>
                  </div>
                </div>
                <hr>
                <div class="card-body" style="margin-top:-25px;">
                  <form action="/addShipment" method="POST" role="form">
                    @csrf
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group mb-4">
                        <label for="example-text-input" class="form-label">Merek Barang</label>
                        <select class="js-select-merek js-merek form-control" required name="brg_id" id="brg_id">
                          <option></option>
                          @foreach ($barang as $nama)
                          <option value="{{$nama->id}}">{{$nama->merek}}</option>
                          @endforeach
                        </select>
                      </div>
                    </div>
                    <div class="col-md-6">
                      <label for="example-text-input" class="form-label">Jumlah Barang</label>
                      <div class="form-group input-group input-group-outline">
                        <input class="form-control" required autocomplete="off" type="number" name="jumlah" placeholder="Jumlah barang yang akan dikirim">
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group input-group input-group-outline mb-4 ">
                        <label for="example-text-input" class="form-label">Nama Penerima</label>
                        <input class="form-control" required autocomplete="off" type="text" name="penerima">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-group input-group input-group-outline">
                        <label for="example-text-input" class="form-label">Nomor Telepon Penerima</label>
                        <input class="form-control" required autocomplete="off" type="text" name="notelp_penerima">
                      </div>
                    </div>
                    <div class="col-md-12">
                      <div class="form-group input-group input-group-outline mb-4">
                        <label for="example-text-input" class="form-label">Alamat Pengiriman</label>
                        <input class="form-control" required autocomplete="off" type="text" name="alamat_kirim">
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-10">
                        <button class="form-control btn btn-md btn-info col-md-10" type="submit"><i class="fa fa-truck"></i> | KIRIM BARANG </button>
                      </div>
                      <div class="col-md-2">
                        <a class="form-control btn btn-md btn-secondary col-md-2" href="pengiriman" data-bs-toggle="modal" data-bs-target="#hapus">
...
$(document).ready(function() {
    $('.js-select-merek').select2({
      id: '-1',
      placeholder: "Pilih merek yang akan dikirim...",
      allowClear: true
    });
});
